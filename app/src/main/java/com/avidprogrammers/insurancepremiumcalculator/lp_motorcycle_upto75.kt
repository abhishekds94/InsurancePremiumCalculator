 ...
 */
class lp_motorcycle_upto75 : AppCompatActivity(), ConnectivityReceiverListener {
    var conn: ConnectivityReceiver? = null
    var checkingStatus: CheckingStatus? = null
    private val mAdView: AdView? = null
    var shareBtn: CircleButton? = null
    var file: File? = null
    var lppaupto75_paod_value: TextView? = null
    var lppaupto75_act: TextView? = null
    var lppaupto75_tax: TextView? = null
    var lppaupto75_total_premium: TextView? = null
    ...
    protected override fun onCreate(savedInstanceState: Bundle?) {
        ...
        checkfunction(this@lp_motorcycle_upto75)
...
/*        mAdView = (AdView) findViewById(R.id.adView);
        ...
        findViewById<View>(R.id.homebtnupto75).setOnClickListener(listener_homebtnupto75)
        val b: Bundle = getIntent().getExtras()!!
        lppaupto75_act = findViewById<View>(R.id.lppaupto75_act_value) as TextView?
        lppaupto75_paod_value = findViewById<View>(R.id.lppaupto75) as TextView?
        lppaupto75_tax = findViewById<View>(R.id.lppaupto75_tax_value) as TextView?
        lppaupto75_total_premium = findViewById<TextView>(R.id.lppaupto75_total_value)
        lppaupto75_act!!.text = b.getCharSequence("lp_motocyc_calc_upto75_act")
        lppaupto75_paod_value!!.text = b.getCharSequence("lp_motocyc_calc_upto75_paod")
        lppaupto75_tax!!.text = b.getCharSequence("lp_motocyc_calc_upto75_tax")
        val act = lppaupto75_act!!.text.toString().toInt()
        val paodv = lppaupto75_paod_value!!.text.toString().toInt()
        val tax = Math.round((act + paodv) * 1.18).toInt()
        lppaupto75_total_premium!!.text = tax.toString()
        shareBtn = findViewById<View>(R.id.sharebtnupto75) as CircleButton?
        shareBtn!!.setOnClickListener {
            val checker = PermissionsChecker(this@lp_motorcycle_upto75)
            if (checker.lacksPermissions(*PermissionsChecker.REQUIRED_PERMISSION)) {
                PermissionsActivity.startActivityForResult(
                    this@lp_motorcycle_upto75,
    ...
    var listener_homebtnupto75 = View.OnClickListener {
        val intent = Intent(this@lp_motorcycle_upto75, home_activity::class.java)
    ...
    private fun settingUpPDF(document: Document) {
        ...
        try {
            ...
            table.addCell(pdfPCell)
            p = Paragraph()
            p.add(Chunk("Rs. " + lppaupto75_act!!.text.toString()))
            pdfPCell = PdfPCell()
            pdfPCell.borderColor = BaseColor.WHITE
            pdfPCell.addElement(p)
            table.addCell(pdfPCell)
            ...
            table.addCell(pdfPCell)
            p = Paragraph()
            p.add(Chunk("Rs. " + lppaupto75_paod_value!!.text.toString()))
            pdfPCell = PdfPCell()
            pdfPCell.borderColor = BaseColor.WHITE
            pdfPCell.addElement(p)
            table.addCell(pdfPCell)
            ...
            table.addCell(pdfPCell)
            p = Paragraph()
            p.add(Chunk("Rs. " + lppaupto75_total_premium!!.text.toString(), white))
            pdfPCell = PdfPCell()
            pdfPCell.borderColor = BaseColor.BLACK
            pdfPCell.backgroundColor = BaseColor.BLACK
            pdfPCell.addElement(p)
